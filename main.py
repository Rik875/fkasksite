...
@app.route("/")
def main_page():
    return '<h1>Привет! Здесь ты можешь узнать пару фактов о технологической зависимости</h1> \n <a href="/random_fact">Посмотреть случайный факт!</a>'
...
@app.route("/Coin")
def coin():
    return '<a href="/Coin_Tossing">подбросить монетку</a>'

@app.route("/Coin_Tossing")
def coin_random():
    return f'<p1>{random.choice(["орел", "решка"])}</p1>'
